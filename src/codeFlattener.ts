...
interface SymbolInfo {
    ...
    parent?: string;    // Parent symbol (if method in a class)
    documentation?: string; // JSDoc or other comments
}

/**
 * Interface for storing information about source files
 */
interface FileInfo {
 ...
 * Note: Most of these options are now hardcoded in the simplified UI, with only a few exposed as settings
 */
...
`;
     ...
     */
    private detectDependencies(content: string, filePath: string): string[] {
...
`;
     ...
     */
    private analyzeFileSymbols(content: string, filepath: string): void {
     ...
     */
    private getHighlightLanguage(filepath: string): string {
        const filename = path.basename(filepath).toLowerCase();
        const ext = path.extname(filepath).toLowerCase();
